...
public class MockServer {
    ...
    private void startWebServer() throws Exception {
        ...
        addRestServices(handler, testScenarioRepository, templateRepository, journalRepository,gsakRepo, kafkaServer.getLocalProducer(), kafkaServer.getKafkaAdminClient());
    ...
    protected void addRestServices(HandlerContainer handler, DelegatingTestscenarioBuilderRepository testScenarioRepository,
                                   DelegatingTestscenarioTemplateRepository templateRepository, JournalRepository journalRepository,
                                   GsakRepo gsakRepo, LocalKafkaProducer localKafkaProducer, AdminClient kafkaAdminClient) {
        new RestConfig(handler, templateRepository).setup(testScenarioRepository, journalRepository, gsakRepo, localKafkaProducer, kafkaAdminClient);
    }
